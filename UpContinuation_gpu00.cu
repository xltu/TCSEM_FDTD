															 ...
															 const ModelPara *d_mp)
{
  ...
  if(ix < L/2+1 && iy < M)
  {
  	...
  	WcuComplex a;
  	if(iy<M/2+1)
	    v=(2.0*PI*iy)/(M*d_mp->dymin);
	  else
	    v=-(2.0*PI*(M-iy))/(M*d_mp->dymin);
	  
	  u=(2.0*PI*ix)/(L*d_mp->dxmin);
	  ...
	  else
	  {
	  	...
	  	upcc = exp(-d_mp->dzmin/2.0*sqrt(u*u+v*v))*rsqrt(u*u+v*v);
			...
			//a=iHilbert(FBzC[iy*(L/2+1)+ix])/(M*L);
			a=iHilbert(FBzC[iy*(L/2+1)+ix]);
	  	FBxC[iy*(L/2+1)+ix]=u*upcc*a;
	  	FByC[iy*(L/2+1)+ix]=v*upcc*a;
	  } 
  }
}
											 ...
											 cudaStream_t *stream)
{
  ...
  M=2*MP->Any_New+MP->Ny;
	...
	cudaEvent_t Event;
	print_CUDA_error_if_any( cudaEventCreate( &Event ), 510031 );
/*	
//--------------------------------------------------------------------------------------------------  
													// for test, do spline interpolation on CPU 
