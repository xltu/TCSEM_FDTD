...
The transmitter could also be an bipole of arbitrary orentation. In that case, the second integer representing the orentation of 
the transmitter should be set to 4. The thrid to the fifth numbers are float type numbers denoting the position of the bipole
center, and the sixth number is a float type number denoting the bipole length in meter. However the seventh number is the
azimuth of the transmitter orientation in degree. The eighth number represents the dip angle of the bipole in degree. 
The transmitter azimuth is defined to be the horizontal rotation of the transmitter antenna from the x axis, positive towards y. 
So an azimuth of 0 means the antenna points along x, while an angle of 90 degrees means the antenna points along
y. The transmitter dip angle is positive down from the azimuth angle. The ninth number is the number of receivers recording the 
current transmitter's signal. Following that is an array of receiver IDs.

The transmitter could also be two bipoles of arbitrary orentation. In that case, the second integer representing the orentation 
of the transmitter should be set to 5. And then the 3rd-8th float numbers represent the parameters [x0, y0, z0, Len, azimuth, dip] 
for the first bipole, the 9th-14th float numbers represent the parameters [x0, y0, z0, Len, azimuth, dip] for the second bipole. 
The 15th number is a integer representing the number of transmitters recording the current transmitter's signal. Following 
that is an array of receiver IDs. 
