...
inline __device__ realw CubicSpline_1d4p( realw *x, realw *y, const realw t )
{
	...
	const realw v1=2.0f*(x[2]-x[0]);
	const realw v2=2.0f*(x[3]-x[1]);
	const realw u1=6.0f*( (y[2]-y[1])/h1 - (y[1]-y[0])/(x[1]-x[0]) );
	const realw u2=6.0f*( (y[3]-y[2])/(x[3]-x[2]) - (y[2]-y[1])/h1 );
	const realw z1= (v2*u1 - h1*u2)/(v1*v2 - h1*h1);
	const realw z2= (v1*u2 - h1*u1)/(v1*v2 - h1*h1);
	
	return z2/(6.0f*h1)*MyPow(t-x[1],3) + z1/(6.0f*h1)*MyPow(x[2]-t,3) + (y[2]/h1 - z2/6.0f * h1)*(t-x[1]) + (y[1]/h1 - z1/6.0f * h1)*(x[2]-t);
}

// nr and base are passed by value, in constant mem
__global__ void ExSpatialInterp( cudaPitchedPtr Ex, const ModelPara *mp, const ModelGrid *MG, 
																RxPos *xyzRx, int *RxLst, realw *ef0, int nr, int Base) 
{
	...
	if(ix<nr)
	{
		...
		if( ( get_global_cr(&(MG->Y_Bzold)[(xyzRx->iy)[ir]-1]) + get_global_cr(&(MG->dy)[(xyzRx->iy)[ir]-1])/2.0f ) > get_global_cr(&(xyzRx->y)[ir]) )
			iskep2=(xyzRx->iy)[ir]-2;
		else
			iskep2=(xyzRx->iy)[ir]-1;
			
		// z direction interpolation
		if( ( get_global_cr(&(MG->Z)[(xyzRx->iz)[ir]-1]) + get_global_cr(&(MG->dz)[(xyzRx->iz)[ir]-1])/2.0f ) > get_global_cr(&(xyzRx->z)[ir]) )
		...
		for(int ij=0;ij<4;ij++)
			t1[ij]=(MG->Y_Bzold)[iskep2+ij]-(MG->dy)[iskep2+ij]/2.0f;	
		...
		for(int ik=0; ik<4; ik++)
		{
			for(ij=0; ij<4; ij++)
			{
				...
				row = (realw *)((char *)Ex.ptr + ( (ik+iskep3) * (mp->M+1) + ij + iskep2 ) * Ex.pitch);		
				Y1[ij]=CubicSpline_1d4p( &((MG->X_Bzold)[iskep1]), &row[iskep1], get_global_cr(&(xyzRx->x)[ir]) );
			}
		...
		#pragma unroll
		for(ij=0;ij<4;ij++)
			t1[ij]=(MG->Z)[iskep3+ij]-(MG->dz)[iskep3+ij]/2.0f;	
...
// nr and base are passed by value, in constant mem
__global__ void EySpatialInterp( cudaPitchedPtr Ey, const ModelPara *mp, const ModelGrid *MG, 
																RxPos *xyzRx, int *RxLst, realw *ef0, int nr, int Base) 
{
	volatile unsigned int ix = blockIdx.x * blockDim.x + threadIdx.x;
	if(ix<nr)
	{
		...
		int iskep1,iskep2,iskep3;
		// interpolation along x direction
		if( ( get_global_cr(&(MG->X_Bzold)[(xyzRx->ix)[ir]-1]) + get_global_cr(&(MG->dx)[(xyzRx->ix)[ir]-1])/2.0f ) > get_global_cr(&(xyzRx->x)[ir]) )
			iskep1=(xyzRx->ix)[ir]-2;
		else
		...
		#pragma unroll
		for(int ij=0;ij<4;ij++)
			t1[ij]=(MG->X_Bzold)[iskep1+ij]-(MG->dx)[iskep1+ij]/2.0f;	
		...
		for(int ik=0; ik<4; ik++)
		{
			...
			{
				// x direction interpolation
				row = (realw *)((char *)Ey.ptr + ( (ik+iskep3) * (mp->M) + ij + iskep2 ) * Ey.pitch);		
				Y1[ij]=CubicSpline_1d4p( t1, &row[iskep1], get_global_cr(&(xyzRx->x)[ir]) );
			}
			// y direction interpolation
...
// nr and base are passed by value, in constant mem
__global__ void EzSpatialInterp( cudaPitchedPtr Ez, const ModelPara *mp, const ModelGrid *MG, 
																RxPos *xyzRx, int *RxLst, realw *ef0, int nr, int Base) 
{
	volatile unsigned int ix = blockIdx.x * blockDim.x + threadIdx.x;
	if(ix<nr)
	{
		...
		#pragma unroll
		for(int ij=0;ij<4;ij++)
			t2[ij]=(MG->Y_Bzold)[iskep2+ij]-(MG->dy)[iskep2+ij]/2.0f;	
		...
		for(int ik=0; ik<4; ik++)
		{
			...
			{
				// x direction interpolation
				row = (realw *)((char *)Ez.ptr + ( (ik+iskep3) * (mp->M+1) + ij + iskep2 ) * Ez.pitch);		
				Y1[ij]=CubicSpline_1d4p( t1, &row[iskep1], get_global_cr(&(xyzRx->x)[ir]) );
			}
			// y direction interpolation
...
// nr and base are passed by value, in constant mem
__global__ void BxSpatialInterp( cudaPitchedPtr Bx, const ModelPara *mp, const ModelGrid *MG, 
																RxPos *xyzRx, int *RxLst, realw *ef0, int nr, int Base) 
{
	volatile unsigned int ix = blockIdx.x * blockDim.x + threadIdx.x;
	if(ix<nr)
	{
		...
		for(int ik=0; ik<4; ik++)
		{
			...
			{
				// x direction interpolation
				row = (realw *)((char *)Bx.ptr + ( (ik+iskep3) * (mp->M) + ij + iskep2 ) * Bx.pitch);		
				Y1[ij]=CubicSpline_1d4p( t1, &row[iskep1], get_global_cr(&(xyzRx->x)[ir]) );
			}
			// y direction interpolation
			Y2[ik]=CubicSpline_1d4p( &(MG->Y_Bzold)[iskep2], Y1, get_global_cr(&(xyzRx->y)[ir]) );
...
// nr and base are passed by value, in constant mem
__global__ void BySpatialInterp( cudaPitchedPtr By, const ModelPara *mp, const ModelGrid *MG, 
																RxPos *xyzRx, int *RxLst, realw *ef0, int nr, int Base) 
{
	volatile unsigned int ix = blockIdx.x * blockDim.x + threadIdx.x;
	if(ix<nr)
	{
		...
		for(int ik=0; ik<4; ik++)
		{
			...
			{
				// x direction interpolation
				row = (realw *)((char *)By.ptr + ( (ik+iskep3) * (mp->M+1) + ij + iskep2 ) * By.pitch);		
				Y1[ij]=CubicSpline_1d4p( &(MG->X_Bzold)[iskep1], &row[iskep1], get_global_cr(&(xyzRx->x)[ir]) );
			}
			// y direction interpolation
			Y2[ik]=CubicSpline_1d4p( t1, Y1, get_global_cr(&(xyzRx->y)[ir]) );
...
// nr and base are passed by value, in constant mem
__global__ void BzSpatialInterp( cudaPitchedPtr Bz, const ModelPara *mp, const ModelGrid *MG, 
																RxPos *xyzRx, int *RxLst, realw *ef0, int nr, int Base) 
{
	volatile unsigned int ix = blockIdx.x * blockDim.x + threadIdx.x;
	if(ix<nr)
	{
		...
		for(int ik=0; ik<4; ik++)
		{
			...
			{
				// x direction interpolation
				row = (realw *)((char *)Bz.ptr + ( (ik+iskep3) * (mp->M) + ij + iskep2 ) * Bz.pitch);		
				Y1[ij]=CubicSpline_1d4p( &(MG->X_Bzold)[iskep1], &row[iskep1], get_global_cr(&(xyzRx->x)[ir]) );
			}
			// y direction interpolation
			Y2[ik]=CubicSpline_1d4p( &(MG->Y_Bzold)[iskep2], Y1, get_global_cr(&(xyzRx->y)[ir]) );
		...
		#pragma unroll
		for(ij=0;ij<4;ij++)
			Y1[ij]=(MG->Z)[iskep3+ij]-(MG->dz)[iskep3+ij]/2.0f;	
							...
							realw *ef0, int Base)
{
	...
	{
		//Ex component
		ExSpatialInterp<<<(nr+BLOCKSIZE1-1)/BLOCKSIZE1,BLOCKSIZE1>>>( DPtrs->Ex, d_mp, d_MG, 
	...
	{
		im=tRx.isComp[0] * nr;
		EySpatialInterp<<<(nr+BLOCKSIZE1-1)/BLOCKSIZE1,BLOCKSIZE1>>>( DPtrs->Ey, d_mp, d_MG, 
	...
	if( tRx.isComp[2] )
	{
		...
		EzSpatialInterp<<<(nr+BLOCKSIZE1-1)/BLOCKSIZE1,BLOCKSIZE1>>>( DPtrs->Ez, d_mp, d_MG, 
																d_xyzRx, RxLst, ef0, nr, Base+im); 
	}
	return 0;
}
							...
							realw *ef0, int Base)
{
	...
	{
		im=tRx.nefcmp*nr;
		BxSpatialInterp<<<(nr+BLOCKSIZE1-1)/BLOCKSIZE1,BLOCKSIZE1>>>( DPtrs->Bx, d_mp, d_MG, 
																d_xyzRx, RxLst, ef0, nr, Base+im); 
	}
	//By
	if( tRx.isComp[4] )
	{
		...
		BySpatialInterp<<<(nr+BLOCKSIZE1-1)/BLOCKSIZE1,BLOCKSIZE1>>>( DPtrs->By, d_mp, d_MG, 
																d_xyzRx, RxLst, ef0, nr, Base+im); 
	}
	//Bz
	if( tRx.isComp[5] )
	{
		...
		BzSpatialInterp<<<(nr+BLOCKSIZE1-1)/BLOCKSIZE1,BLOCKSIZE1>>>( DPtrs->Bz, d_mp, d_MG, 
																d_xyzRx, RxLst, ef0, nr, Base+im); 
	}
	return 0;
}

//TODO incorporate Tx dependent Rxs
int EFInterp2Rx(int *ne_efs, int *nb_efs, RxTime tRx, TxPos xyzTx, RxPos *d_xyzRx, int *RxLst,
								...
								realw Tpof[2][3], realw *efs, realw *ef0)
{
	realw l1;
	//-------------------------------------electric field----------------------------------------------------------------
	if( *ne_efs < tRx.Nt)
	{
		...
		if( tRx.Times[*ne_efs] <= Tpof[0][2] && tRx.Times[*ne_efs] > Tpof[0][1] )
		{
			...
			EFXInterp(DPtrs, d_MG, d_mp, tRx, xyzTx.nr, d_xyzRx, RxLst, ef0, 0);
		}
		...
		else if( tRx.Times[*ne_efs] <= Tpof[0][1]  && tRx.Times[*ne_efs] > Tpof[0][0] )  	
		{
			...
			EFXInterp(DPtrs, d_MG, d_mp, tRx, xyzTx.nr, d_xyzRx, RxLst, ef0, tRx.NComp*xyzTx.nr);
			...
			while(*ne_efs < tRx.Nt && tRx.Times[*ne_efs] <= Tpof[0][1] )
			{
				...
				EFTimeInterp<<<(xyzTx.nr+BLOCKSIZE1-1)/BLOCKSIZE1,BLOCKSIZE1>>>( ef0, efs, xyzTx.nr, tRx.Nt, *ne_efs, tRx.nefcmp, tRx.NComp, 1.0f-l1, l1);
	...
	if( *nb_efs < tRx.Nt && ( tRx.isComp[3] || tRx.isComp[4] || tRx.isComp[5] ) )
	{
		...
		if( tRx.Times[*nb_efs] <= Tpof[1][2] && tRx.Times[*nb_efs] > Tpof[1][1] )
		{
			...
			BFXInterp(DPtrs, d_MG, d_mp, tRx, xyzTx.nr, d_xyzRx, RxLst, ef0, 0);
		}
		// next sampling time is in the middle of the previous and current time iteration
		// specially interpolate the E field of the current time step to the receiver locations
		// temporarily interpolate the E field to the next sampling time
		...
		else if( tRx.Times[*nb_efs] <= Tpof[1][1]  && tRx.Times[*nb_efs] > Tpof[1][0] )  	
		{
			...
			BFXInterp(DPtrs, d_MG, d_mp, tRx, xyzTx.nr, d_xyzRx, RxLst, ef0, tRx.NComp*xyzTx.nr);
							
			// temporial interpolation to the rx sampling time
			while( *nb_efs < tRx.Nt && tRx.Times[*nb_efs] <= Tpof[1][1] )
			{
				l1=(tRx.Times[*ne_efs]-Tpof[1][0])/(Tpof[1][1]-Tpof[1][0]);
				BFTimeInterp<<<(xyzTx.nr+BLOCKSIZE1-1)/BLOCKSIZE1,BLOCKSIZE1>>>( ef0, efs, xyzTx.nr, tRx.Nt, *nb_efs, tRx.nefcmp, tRx.NComp, 1.0f-l1, l1);
